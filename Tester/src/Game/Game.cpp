...
void Game::checkInputs(gllib::Animation* player)
{
    ...
    if (Input::getKeyPressed(Key_S))
    {
        // S
        transform.position.y += 2.0f;
        if (!collisionManager->checkCollision(transform)&& !rock->isColliding(transform))
        {
            player->move({0.f, speed, 0.f});
        }
    }
    if (Input::getKeyPressed(Key_S) || Input::getKeyPressed(Key_W) ||
        Input::getKeyPressed(Key_A) || Input::getKeyPressed(Key_D))
    {
        player->update();
...
{
    if (player->isColliding(rock->getTransform())) return;
